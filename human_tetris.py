"""
ASSIGNMENT 2 FINAL ULTIMATE: AI EXER-GAME
Tính năng mới (Sprint 15):
- Power-ups: Trái tim (Hồi máu) & Khiên (Bất tử).
- Shield Visual: Hiệu ứng vòng bảo vệ xoay quanh người.
- Item Rarity: Tỉ lệ rơi vật phẩm hiếm.
"""
import cv2
...
def play_sound(type):
    def run():
        ...
        elif type == "bomb": winsound.Beep(150, 300)
        elif type == "click": winsound.Beep(800, 100)
        elif type == "hover": winsound.Beep(600, 30)
        elif type == "shield_up": winsound.Beep(3000, 200) # Tiếng khiên
        elif type == "shield_down": winsound.Beep(500, 500)
        elif type == "heal": winsound.Beep(600, 100); winsound.Beep(800, 100) # Tiếng hồi máu
        elif type == "gameover": 
...
class Button:
    def __init__(self, text, x, y, w, h, color=(0, 255, 0)):
        self.text = text; self.rect = (x, y, w, h); self.color = color
        self.hover_timer = 0; self.required_time = 20; self.is_triggered = False
    ...
    def draw(self, img):
        ...
        if self.hover_timer > 0:
            ...
            overlay = img.copy(); cv2.rectangle(overlay, (x, y), (x + fill, y + h), self.color, -1)
...
class ItemManager:
    ...
    def update(self, w, h):
        ...
        if self.timer > 60: 
            self.timer = 0
            rand = random.random()
            # Tỉ lệ rơi đồ: 50% Tiền, 30% Bom, 15% Khiên, 5% Tim
            if rand < 0.5: type = 'coin'; radius = 25
            elif rand < 0.8: type = 'bomb'; radius = 35
            elif rand < 0.95: type = 'shield'; radius = 30
    ...
    def draw(self, img):
        for item in self.items:
            ...
            elif t == 'shield': c = (255, 0, 0); txt = "S" # Xanh dương
            elif t == 'heart': c = (0, 0, 255); txt = "H" # Tim đỏ
            ...
            for idx in pts:
                ...
                if math.sqrt((px - item['x'])**2 + (py - item['y'])**2) < item['radius'] + 15:
                    ...
                    if item['type'] == 'coin':
                        ...
                        if has_shield: # Có khiên thì đỡ được bom
                            play_sound("shield_down"); add_text("BLOCKED!", item['x'], item['y'], (255, 0, 0))
                        else:
                            hit['damage'] = True; play_sound("bomb"); add_text("OUCH!", item['x'], item['y'], (0, 0, 255))
...
calib_timer = 60; items = ItemManager()
btn_start = Button("START GAME", 200, 300, 240, 60); btn_restart = Button("RESTART", 220, 380, 200, 60, (0,255,255))
...
def reset_game():
    ...
    calib_timer = 60; game_state = "CALIBRATION"; items = ItemManager()
    shield_timer = 0 # Reset khiên
...
with mp_pose.Pose(min_detection_confidence=0.6, min_tracking_confidence=0.6, model_complexity=1) as pose:
    while cap.isOpened():
        ...
        if game_state == "MENU":
            ...
            if results.pose_landmarks and btn_start.check_hover(image, results.pose_landmarks.landmark, w, h): reset_game()
            ...
            if results.pose_landmarks:
                ...
                if shield_timer > 0:
                    shield_timer -= 1
                    # Lấy tâm hông để vẽ vòng tròn bao quanh người
                    hip_x = int((lm[23].x + lm[24].x) * w / 2)
                    hip_y = int((lm[23].y + lm[24].y) * h / 2)
                    # Hiệu ứng nhấp nháy khi sắp hết
                    if shield_timer > 60 or (shield_timer // 5) % 2 == 0:
                        cv2.circle(image, (hip_x, hip_y), 250, (255, 0, 0), 5) # Vòng tròn xanh dương
                        cv2.putText(image, f"SHIELD: {shield_timer//30}", (hip_x-60, hip_y-260), 1, 1, (255,0,0), 2)

                draw_neon_skeleton(image, lm, combo)
                ...
                items.update(w, h)
                items.draw(image)
                hit = items.check_hit(lm, w, h, shield_timer > 0) # Truyền trạng thái khiên vào
                
                if hit['score'] > 0: score += hit['score']
                if hit['heal']: lives = min(3, lives + 1) # Hồi máu (Max 3)
                if hit['shield']: shield_timer = 300 # Kích hoạt 10 giây (300 frames)
                
                if hit['damage']: # Chỉ bị thương nếu không có khiên (đã check trong hàm check_hit)
                    lives -= 1; shake_timer = 10; combo = 0
                    if lives == 0: 
                        game_state = "GAMEOVER"; play_sound("gameover")
                        if score > high_score: high_score = score; save_high_score(high_score); print(f"NEW RECORD: {high_score}")
                ...
                cv2.putText(image, f"TOP: {high_score}", (w - 220, 60), 1, 1.5, (0, 255, 255), 2)
